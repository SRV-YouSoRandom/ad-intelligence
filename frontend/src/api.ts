...
export const useJobPoll = (jobId: string | null, interval = 3000) => {
  ...
  return { job: data, isPolling: !!jobId && data?.status !== 'DONE' && data?.status !== 'FAILED', error, mutate };
};

export const searchBrand = async (payload: { identifier: string; identifier_type: string; countries: string[]; ad_active_status: string; max_ads?: number }) => {
  return fetchApi('/brands/search', {
    method: 'POST',
...
export const generateInsight = async (adId: string) => {
  return fetchApi(`/ads/${adId}/insights/generate`, { method: 'POST' });
};

export const deleteInsight = async (adId: string) => {
  return fetchApi(`/ads/${adId}/insights`, { method: 'DELETE' });
};
